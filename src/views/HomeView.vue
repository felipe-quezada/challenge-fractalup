<template>
  <main class="container">
      ...
      <form class="search-form" @submit.prevent="">
        ...
        <div v-if="checboxFocus" class="checkbox-continents">
          ...
          <button @click="handleOnFocusInput(true)"
            style="position: absolute; top: 20px; right: 20px; border-style: none; cursor: pointer;">X</button>
      ...
      <div v-for="(country, index) in twelvesDisplayCards" :key="country.code">
        ...
        <div @click="handdleSelectClick(country, index)" class="card-country-container"
          ...
          <div :class='`card-country-container-text ${(index === selected.index) ? "active" : ""}`'>
            ...
            <br>
            <div>
              <p><span class="card-country-name">{{ country.name }}</span></p>
    ...
    <section v-if="selected" class="float-card-container">
      <button @click="handdleSelectClick(null, selected.index)" class="button-close-float">x</button>
      <div class="float-card-img" :style="{ backgroundImage: `url(${orderCountry(selected.country.name)})` }">
      </div>
      <section class="float-card-info">
        <div class="float-card-info-card">
          <div class="float-card-info-card-head">
            ...
            <br>
            <div>
              <p><span class="card-country-name">{{ selected.country.name }}</span></p>
              <span class="card-country-continent">{{ selected.country.continent.name }}</span>
            </div>
          </div>
          <ul class="card-country-data">
            <li><span class="float-card-text-enphasis">Capital: </span>{{ selected.country.capital }}</li>
            <br>
            <li><span class="float-card-text-enphasis">Lenguage: </span>{{ selected.country.languages[0].name }}</li>
            <br>
            <li><span class="float-card-text-enphasis">Population: </span>500k pople</li>
            <br>
            <li><span class="float-card-text-enphasis">Currency: </span>{{ selected.country.currency }}</li>
            <br>
            <li><span class="float-card-text-enphasis">Region:</span>
...
query searchCountryByName($searchCountry: String){ countries(filter: {name: {regex: $searchCountry}}){
    code
    name
    continent{
      name
...
const twelvesDisplayCards = computed(() => filteredCountries.data.filter((country) => country.name !== 'Anguilla' && country.continent.name.includes(continent.value)).slice(0, 12))
...
const getCountry = (searchInput) => {
  ...
  filteredCountries.data = dataCountries.value.countries.filter((country) => country.name.includes(searchCountry))
}
const getImageCountry = (countries) => {
  ...
  countries.forEach(country => {
    axios.request({
      ...
      .then(({ data }) => {
        urlImages.value = [...urlImages.value, { country: country.name, url: data.photos[Math.floor(Math.random() * 3)].src.landscape }]
...
const handdleSelectClick = (country, index) => {
  if (index === selected.value.index) { selected.value = false }
